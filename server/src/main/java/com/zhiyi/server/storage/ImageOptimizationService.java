...
public class ImageOptimizationService {
  ...
  public byte[] optimize(byte[] original) throws IOException {
    return optimize(original, EnhanceParams.DEFAULT);
  }

  public byte[] optimize(byte[] original, EnhanceParams p) throws IOException {
    BufferedImage src = ImageIO.read(new ByteArrayInputStream(original));
    if (src == null) throw new IOException("无法解析图片");
    src = toRgb(src);
    BufferedImage result = src;
    result = enhanceContrast(result, p.contrastLowClip(), p.contrastHighClip());
    result = adjustBrightness(result, p.brightnessScale(), p.brightnessOffset());
    result = denoise(result, p.denoiseRadius(), p.denoiseSigmaColor(), p.denoiseSigmaSpace());
    result = sharpen(result, p.sharpenAmount(), p.sharpenBlurSigma(), p.sharpenThreshold());
    result = enhanceVibrance(result, p.vibranceStrength(), p.vibranceLowSatThreshold(), p.vibranceSkinProtectFactor());
    ByteArrayOutputStream out = new ByteArrayOutputStream();
    ImageIO.write(result, "jpg", out);
    return out.toByteArray();
  }

  private BufferedImage adjustBrightness(BufferedImage src, float scale, float offset) {
    RescaleOp op = new RescaleOp(scale, offset, null);
    return op.filter(src, null);
  }

  private BufferedImage enhanceContrast(BufferedImage src, double lowClip, double highClip) {
    ...
    for (int i = 0; i < 256; i++) {
      cum += hist[i];
      if (cum < total * lowClip) pLow = i;
      if (cum < total * highClip) pHigh = i;
    }
  ...
  private BufferedImage denoise(BufferedImage src, int radius, double sigmaColor, double sigmaSpace) {
    int w = src.getWidth(), h = src.getHeight();
    BufferedImage dst = new BufferedImage(w, h, src.getType());
    for (int y = 0; y < h; y++) {
      for (int x = 0; x < w; x++) {
        ...
        for (int dy = -radius; dy <= radius; dy++) {
          for (int dx = -radius; dx <= radius; dx++) {
            ...
            double weight = Math.exp(-dist / (2 * sigmaSpace * sigmaSpace)) * Math.exp(-colorDist / (2 * sigmaColor * sigmaColor));
  ...
  private BufferedImage sharpen(BufferedImage src, float amount, float blurSigma, int threshold) {
    int w = src.getWidth(), h = src.getHeight();
    BufferedImage blurred = gaussianBlur(src, blurSigma);
    BufferedImage dst = new BufferedImage(w, h, src.getType());
    for (int y = 0; y < h; y++) {
      for (int x = 0; x < w; x++) {
        for (int c = 0; c < 3; c++) {
          ...
          int diff = orig - blur;
          if (Math.abs(diff) < threshold) setChannel(dst, x, y, c, orig);
  ...
  private BufferedImage enhanceVibrance(BufferedImage src, float strength, float lowSatThreshold, float skinProtectFactor) {
    int w = src.getWidth(), h = src.getHeight();
    BufferedImage dst = new BufferedImage(w, h, src.getType());
    for (int y = 0; y < h; y++) {
      for (int x = 0; x < w; x++) {
        ...
        float adjusted = sat + strength * (1f - sat) * (sat < lowSatThreshold ? 1.2f : 1f);
        adjusted = adjusted * (1f - skinProtect * skinProtectFactor);
