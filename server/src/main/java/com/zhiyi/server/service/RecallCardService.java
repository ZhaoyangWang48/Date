...
import com.zhiyi.server.repository.TreeMemberRepository;
...
public class RecallCardService {
  ...
  private final TreeMemberRepository treeMemberRepo;
  ...
  public RecallCardService(MemoryRepository memoryRepo, RecallCardRepository recallCardRepo,
                           TreeMemberRepository treeMemberRepo, RecallSummaryAgent summaryAgent) {
    this.memoryRepo = memoryRepo;
    this.recallCardRepo = recallCardRepo;
    this.treeMemberRepo = treeMemberRepo;
  ...
  public Dtos.RecallCardResponse generate(UserAccount user, Long treeHoleId) {
    ...
    if (treeHoleId != null && treeHoleId > 0) requireTreeMember(user.getId(), treeHoleId);
    LocalDateTime lastScan = (treeHoleId == null || treeHoleId <= 0
        ? recallCardRepo.findFirstByUserIdAndTreeHoleIsNullOrderByCreatedAtDesc(user.getId())
        : recallCardRepo.findFirstByUserIdAndTreeHoleIdOrderByCreatedAtDesc(user.getId(), treeHoleId))
  ...
  public List<Dtos.RecallCardResponse> list(UserAccount user, Long treeHoleId) {
    ...
    if (treeHoleId == null || treeHoleId <= 0) {
      ...
      requireTreeMember(user.getId(), treeHoleId);
  ...
  public Dtos.RecallCardResponse detail(UserAccount user, Long cardId) {
    RecallCardEntity card = recallCardRepo.findById(cardId)
        .orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "回忆卡片不存在"));
    if (!card.getUser().getId().equals(user.getId())) {
      throw new ApiException(HttpStatus.FORBIDDEN, "无权查看该回忆卡片");
    }
    return Dtos.RecallCardResponse.from(card);
  }

  private void requireTreeMember(Long userId, Long treeHoleId) {
    treeMemberRepo.findByTreeHoleIdAndUserId(treeHoleId, userId)
        .orElseThrow(() -> new ApiException(HttpStatus.FORBIDDEN, "无权访问该树洞"));
  }
}
