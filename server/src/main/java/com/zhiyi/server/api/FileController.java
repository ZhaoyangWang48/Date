...
import com.zhiyi.server.agent.ImageAnalysisService;
import com.zhiyi.server.api.Dtos.ImageOptimizeResponse;
import com.zhiyi.server.api.Dtos.SmartEnhanceResponse;
...
public class FileController {
  ...
  @PostMapping("/enhance/smart")
  public ApiResponse<SmartEnhanceResponse> smartEnhance(@RequestBody Map<String, Object> body) {
    try {
      String imageUrl = (String) body.get("imageUrl");
      log.info("Smart enhance: {}", imageUrl);
      byte[] bytes = storage.read(imageUrl);
      ImageAnalysisService.AnalysisResult analysis = analysisService.analyze(imageUrl);
      log.info("Kimi analysis: {} → params: {}", analysis.analysis(), analysis.params());
      bytes = optimizer.optimize(bytes, analysis.params());
      String newUrl = storage.store(bytes, deriveName(imageUrl, "smart"));
      return ApiResponse.ok("智能增强完成：" + analysis.analysis(),
          new SmartEnhanceResponse(newUrl, analysis.analysis()));
    } catch (Exception e) {
      log.error("Smart enhance failed: {}", e.getMessage());
      return ApiResponse.ok("智能增强失败，请稍后重试",
          new SmartEnhanceResponse(null, e.getMessage()));
    }
  }
